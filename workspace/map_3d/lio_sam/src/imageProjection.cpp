...
private:
    ...
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubLaserCloudin;
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr tmpVlpCloudIn;
...
public:
            ...
            ParamServer("lio_sam_imageProjection", options), deskewFlag(0)
    {
        ...
        pubLaserCloudin = create_publisher<sensor_msgs::msg::PointCloud2>(
            "/LaserCloudin", 1);
    ...
    void allocateMemory()
    {
        ...
        tmpVlpCloudIn.reset(new pcl::PointCloud<pcl::PointXYZI>());
    ...
    bool cachePointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr& laserCloudMsg)
    {
        ...
        {
            pcl::moveFromROSMsg(currentCloudMsg, *tmpVlpCloudIn);
            int cloudSize = tmpVlpCloudIn->points.size();
            float startOri = -atan2(tmpVlpCloudIn->points[0].y, tmpVlpCloudIn->points[0].x);
            float endOri = -atan2(tmpVlpCloudIn->points[cloudSize - 1].y,
                                    tmpVlpCloudIn->points[cloudSize - 1].x) + 2 * M_PI;
            double scanPeriod = 0.1;
            double start_time = stamp2Sec(currentCloudMsg.header.stamp);
            ...
            PointXYZIRT point;
            std::vector<int> rings(20, 0);
            for (size_t i = 0; i < tmpVlpCloudIn->size(); i++)
            {
                auto &src = tmpVlpCloudIn->points[i];
                point.x = src.x;
                point.y = src.y;
                point.z = src.z;
                // Cancel ground scanning
                // if(point.z < -0.43) continue;
                ...
                if (roundedAngle > 0)
                {
                    point.ring = roundedAngle;
                }
                else 
                {
                    point.ring = roundedAngle + (16 - 1);
                }
                ...
                point.time = (ori - startOri) / (endOri - startOri) * scanPeriod + start_time;
        ...
        // tempCloud.header.stamp = currentCloudMsg.header.stamp;
        // tempCloud.header.frame_id = currentCloudMsg.header.frame_id;
        // pubLaserCloudin->publish(tempCloud);
